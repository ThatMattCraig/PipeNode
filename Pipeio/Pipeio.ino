...
int countDown = 60;  // Countind down 2 minutes
...
void loop() 
{
  ...
  String tempStringC = String(objTempC);
  ...
  if(objTempF >= 33 && faren  == false){
    tft.setCursor(curX,curY);
    tft.setTextSize(2);
    tft.println("Object Temp :");
    tft.println(objTempC);
    client.publish("mytopic/test",tempStringC); // You can activate the retain flag by setting the third parameter to true
  }

  // if freezing or below raise alarm
    if(objTempF < 33.00 && faren == false){
      client.publish("mytopic/test", "Freezing!!", tempStringC);
      tft.setCursor(curX,curY);
      tft.println("Object Temp");
      tft.println(objTempC);
      tft.println("Freezing!!!");//alarms here
...
void handler(Button2& btn) {
    if (btn.getClickType() == DOUBLE_CLICK){ 
            faren = !faren;
